...
const roleKeys = [
  { title: 'Team.r1Title', body: 'Team.r1Body' },
  { title: 'Team.r2Title', body: 'Team.r2Body' },
  { title: 'Team.r3Title', body: 'Team.r3Body' },
  { title: 'Team.r4Title', body: 'Team.r4Body' },
] as const;
      ...
      <div className="wrap" style={{ display: 'grid', gridTemplateColumns: '0.95fr 1.05fr', gap: 80, alignItems: 'center' }}>
        <div style={{ position: 'relative' }}>
          <div className="imgwrap" style={{ aspectRatio: '5/4', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-lg)' }}>
            <img src={img('boardroom', 1200)} alt={t('Team.imageAlt')} />
          </div>
          <div style={{
            ...
            <div style={{ fontFamily: 'var(--font-sans)', fontWeight: 600, fontSize: 44, letterSpacing: '-0.02em', lineHeight: 1 }}>
              {t('Team.badgeYears')}<span style={{ fontSize: 24 }}>{t('Team.badgePlus')}</span>
            </div>
            <div className="small" style={{ color: 'var(--on-dark-2)', marginTop: 6, maxWidth: 150 }}>{t('Team.badgeNote')}</div>
          </div>
        </div>

        <div>
          <div className="eyebrow"><span className="tick" />{t('Team.eyebrow')}</div>
          <h2 className="h2" style={{ margin: '22px 0 0' }}>{t('Team.title')}</h2>
