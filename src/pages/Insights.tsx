...
export default function InsightsPage() {
  ...
  return (
    <>
      <Helmet>
        <title>{`${t('Pages.insightsTitle')} — Anchor Risk Transfer`}</title>
      </Helmet>
      <PageHero
        crumb={t('Pages.insightsCrumb')}
        eyebrow={t('Pages.insightsHeroEyebrow')}
        imgKey="cityNight"
        title={t('Pages.insightsHeroTitle')}
        sub={t('Pages.insightsHeroSub')}
      />
      ...
      <section className="section" style={{ background: 'var(--bone)' }}>
        <div className="wrap">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 32, marginBottom: 56 }}>
              ...
              >
                ...
                <div style={{ position: 'relative', height: '100%', minHeight: 460, padding: 44, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', color: '#fff' }}>
                  ...
                  <span className="mono" style={{ color: 'var(--navy-200)', letterSpacing: '0.1em' }}>{t('InsightsTeaser.minRead', { n: featured.readingMins })}</span>
                </div>
              </LocaleLink>

              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <div className="eyebrow"><span className="tick" />{t('Pages.insightsLatest')}</div>
          ...
          {rest.length > 4 && (
            <>
              <div className="eyebrow" style={{ marginBottom: 24 }}><span className="tick" />{t('Pages.insightsArchive')}</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0,1fr))', gap: 24 }}>
                {rest.slice(4).map((p) => (
                  ...
                  >
                    ...
                    <div className="mono" style={{ marginTop: 18, color: 'var(--fg-3)' }}>{fmtDate(p.date, locale)} · {p.readingMins} {t('Pages.insightMin')}</div>
                  </LocaleLink>
                ))}
              </div>
            </>
          ...
          {posts.length === 0 && (
            <p className="lead">{t('Pages.insightsEmpty')}</p>
          )}
        </div>
      </section>

      <CTAStrip
        title={t('Pages.insightsCtaTitle')}
        sub={t('Pages.insightsCtaSub')}
      />
    </>
  );
}
